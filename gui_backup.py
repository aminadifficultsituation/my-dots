...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Log Display
        self.log_label = ctk.CTkLabel(self, text="System Logs:", font=("Roboto", 14))
        self.log_label.pack()
    ...
    def run_backup(self):
        ...
        try:
            ...
            # Use check=True so Python catches Git errors!
            result = subprocess.run([script_path], env=my_env, capture_output=True, text=True, check=True)
            
            self.update_logs()
            messagebox.showinfo("Success", "GitHub Sync Completed!")
            
        except subprocess.CalledProcessError as e:
            # This will now show the REAL error from the terminal
            error_msg = e.stderr if e.stderr else "Unknown Git Error"
